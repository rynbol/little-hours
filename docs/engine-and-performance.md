...
A still pet (asleep, sitting, settling) casts into the cached sun shadow, which then redraws once for each new resting place; a walking or carried pet keeps only its soft contact shade, so the cached map never shows a stale pose. The pet never receives the cached shadow. It stands on its bed's cushion, on the top rug under it (a flattened rug lowers it) or on the floor.
