...
try {
  ...
  motion.matches = false; motion.emit('change', { matches: false }); advance(2);
  {
    ...
    room.setDoorActive('garden'); canvas.emit('pointerleave'); advance(3);
    assert.ok(scene.getTransformNodeByName('door-hinge-garden').rotation.y < -.5, 'the selected door stays open after the pointer leaves');
    ...
    let arrivedGarden = false;
    assert.equal(room.walkToDoor('garden', () => { arrivedGarden = true; }), true, 'the built door starts an avatar trip');
    ...
    for (let i = 0; i < 900 && !arrivedGarden; i++) advance();
    assert.equal(arrivedGarden, true, 'the trip reports arrival once it reaches the door');
    assert.equal(diagnostics().companion.state, 'at-door'); assert.ok(diagnostics().companion.x > 7);
    ...
    let arrivedLoft = false;
    assert.equal(room.walkToDoor('loft', () => { arrivedLoft = true; }), true, 'a built loft starts a stair trip');
    for (let i = 0; i < 1200 && !arrivedLoft; i++) advance();
    assert.equal(arrivedLoft, true, 'the avatar reaches the upper doorway');
