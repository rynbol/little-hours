...
try {
  ...
  motion.matches = false; motion.emit('change', { matches: false }); advance(2);
  {
    ...
    room.setDoorActive('garden'); canvas.emit('pointerleave'); advance(60);
    assert.ok(Math.abs(scene.getTransformNodeByName('door-hinge-garden').rotation.y + .18) < .03, 'a selected door stays closed until the avatar reaches it');
    ...
    const startingPose = { x: diagnostics().companion.x, z: diagnostics().companion.z };
    advance(45);
    assert.notDeepEqual({ x: diagnostics().companion.x, z: diagnostics().companion.z }, startingPose, 'the avatar visibly leaves its starting place');
    assert.equal(openedGarden, false, 'the door remains shut while the avatar approaches');
    assert.equal(arrivedGarden, false, 'room transition waits until the door');
    let openFrame = 0;
    for (; openFrame < 240 && !openedGarden; openFrame++) advance();
    assert.equal(openedGarden, true, 'the avatar reaches for and opens the door');
    assert.equal(diagnostics().companion.activity, 'door', 'the avatar reaches toward the handle');
    assert.equal(arrivedGarden, false, 'the avatar waits for the door to open');
    const openedAngle = scene.getTransformNodeByName('door-hinge-garden').rotation.y;
    advance(36);
    assert.ok(scene.getTransformNodeByName('door-hinge-garden').rotation.y < openedAngle - .25, 'the door visibly swings open');
    assert.equal(arrivedGarden, false, 'the avatar has not crossed before the opening action completes');
    for (let i = 0; i < 30 && !arrivedGarden; i++) advance();
    assert.equal(arrivedGarden, true, 'the trip reaches the other side at its fixed time');
    assert.equal(diagnostics().companion.state, 'at-door'); assert.ok(diagnostics().companion.x > 8, 'the avatar crosses the threshold');
    assert.equal(diagnostics().companion.moving, false);
    assert.equal(diagnostics().layout.items.length, layoutBefore.items.length, 'door movement leaves furniture unchanged');
    room.setActivity('working');
    for (let i = 0; i < 900 && !diagnostics().companion.atDesk; i++) advance();
    assert.equal(diagnostics().companion.atDesk, true, 'focus intent safely returns the avatar from the porch');
    room.setActivity('idle'); advance(2);
    const houseWithLoft = { ...house, rooms: [...house.rooms, { id: 'loft', name: 'Upstairs hideaway', layout: createLayout('moonlit-greenhouse') }] };
    room.setHouse(houseWithLoft); advance(2);
    room.setDoorActive('loft');
    let arrivedLoft = false, openedLoft = false;
    assert.equal(room.walkToDoor('loft', () => { arrivedLoft = true; }, () => { openedLoft = true; room.setDoorOpen('loft'); }), true, 'a built loft starts a stair trip');
    for (let i = 0; i < 1200 && !arrivedLoft; i++) advance();
    assert.equal(openedLoft, true, 'the avatar opens the upstairs door after climbing the stairs');
    assert.equal(arrivedLoft, true, 'the avatar crosses the upper doorway');
    assert.ok(Math.abs(diagnostics().companion.walkHeight - .95) < 1e-6, 'the avatar climbs to the modeled landing');
    assert.ok(diagnostics().companionModel.root.position.y > 1, 'the rendered avatar is raised onto the loft stairs');
